...
#		elif chunk_id in [b'ICRD', b'IENG', b'ISFT', b'ISTJ']:	# see http://www.pjb.com.au/midi/sfspec21.html#i5
		elif chunk_id in [b'ICRD', b'IENG', b'ISFT', b'ISTJ', b'INAM', b'ltxt', b'\x00\x00\x00\x00']:
	...
	* To write multiple-channels, use a 2-D array of shape
	...
	if markers:	# != None and != []
		...
		for i, c in enumerate(markers):
			marker_position = c['position'] if isinstance(c, dict) else c
			
			s = struct.pack('<iiiiii', 
				i + 1,		   # Unique marker ID
				marker_position, # Marker position
				1635017060,	  # Chunk ID (usually 'data')
				0,			   # Chunk start
				0,			   # Block start
				marker_position  # Sample number
			)
			fid.write(s)
		...
		for i, label in enumerate(labels):
			...
			if len(label_bytes) % 2 == 0:
				label_bytes += b'\x00\x00'
			else:
				label_bytes += b'\x00'
		
			size = len(label_bytes) + 4  # 4 bytes for cue ID
			lbls += struct.pack('<ii', size, i + 1)
			lbls += label_bytes

		if lbls:
			fid.write(b'LIST')
			fid.write(struct.pack('<i', len(lbls) + 4))  # +4 for 'adtl'
			fid.write(b'adtl')
			fid.write(lbls)
